...
type AuditItem = {
  ...
  detail: string;
  meta: string;
  category: "workspace" | "team" | "task" | "file";
...
export default function AuditConsole({ workspaceName, items, summary }: AuditConsoleProps) {
  ...
  const filteredItems = items.filter((item) => {
    ...
    return [item.title, item.detail, item.meta, item.projectName, item.actorName]
      .filter(Boolean)
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 p-6">
      <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <h1 className="mt-4 text-4xl font-black tracking-tight text-slate-950">Audit log ve denetlenebilir is akisi</h1>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
          {workspaceName} icindeki rol degisiklikleri, workspace guncellemeleri ve task hareketleri tek bir denetim
          yuzeyinde okunur. Bu sayfa Synorq’un kontrol ve auditability katmanidir.
        </p>
      </div>
      ...
      <section className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            {scopes.map((item) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                  scope === item.id
                    ? "bg-[linear-gradient(135deg,rgba(99,102,241,0.14),rgba(6,182,212,0.12))] text-indigo-700"
          ...
          <input
            ...
            placeholder="Aksiyon, proje veya kisi ara"
